...
• Security: Run as non-root user and minimize privileges"
    ...
    if [ "$SKIP_PROJECT_CREATION" = false ]; then
        ...
        # Remove the default WeatherForecast controller and model that don't align with exercises
        rm -f Controllers/WeatherForecastController.cs
        rm -f WeatherForecast.cs
    fi

    # Override the default Program.cs to match exercise requirements
...
app.Run();' \
"Container-ready ASP.NET Core application with health checks as required by Exercise 1"
