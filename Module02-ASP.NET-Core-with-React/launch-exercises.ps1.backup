...
function Show-LearningObjectivesInteractive {
    ...
    switch ($Exercise) {
        "exercise01" {
            ...
            Write-Host "  ⚛️ 1. Setting up React with ASP.NET Core"
            Write-Host "  ⚛️ 2. Creating a React SPA frontend"
            Write-Host "  ⚛️ 3. Configuring development proxy"
            Write-Host "  ⚛️ 4. Basic component structure"
            Write-Host ""
        ...
        "exercise02" {
            ...
            Write-Host "  🔄 1. React Router for navigation"
            Write-Host "  🔄 2. State management with Context API"
            Write-Host "  🔄 3. Form handling and validation"
            Write-Host "  🔄 4. Component lifecycle management"
            Write-Host ""
        ...
        "exercise03" {
            ...
            Write-Host "  🚀 1. HTTP client configuration"
            Write-Host "  🚀 2. API integration patterns"
            Write-Host "  🚀 3. Error handling and loading states"
        ...
        "exercise04" {
            ...
            Write-Host "  🐳 1. Containerizing ASP.NET Core API"
            Write-Host "  🐳 2. Containerizing React application"
            Write-Host "  🐳 3. Docker Compose for multi-container setup"
            Write-Host "  🐳 4. Development and production configurations"
            Write-Host ""
...
# Welcome screen
Write-ColorOutput ("━" * 60) -Color Magenta
Write-ColorOutput "🚀 Module 02 - ASP.NET Core with React" -Color Magenta
Write-ColorOutput ("━" * 60) -Color Magenta
Write-Host ""
...
"@
        ...
        if (-not (Test-Path "frontend")) {
            Write-ColorOutput "❌ Frontend directory not found. Run exercise01 first." -Color Red
            exit 1
        }
...
"@
        ...
        if (-not (Test-Path "frontend")) {
            Write-ColorOutput "❌ Frontend directory not found. Run previous exercises first." -Color Red
            exit 1
        }
...
"@
        ...
        if (-not (Test-Path "backend") -or -not (Test-Path "frontend")) {
            Write-ColorOutput "❌ Backend and frontend directories not found. Run previous exercises first." -Color Red
            exit 1
        }
...
Write-Host "🚀 Starting React + ASP.NET Core in development mode..." -ForegroundColor Cyan
...
'@
        ...
        Write-ColorOutput "✅ Exercise 4 setup complete!" -Color Green
        ...
        # Check if Docker is running
        Write-ColorOutput "🐳 Starting Docker containers..." -Color Cyan
        $dockerInfo = docker info 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-ColorOutput "❌ Docker is not running. Please start Docker first." -Color Red
        ...
        Write-ColorOutput "📚 Swagger UI: http://localhost:5001/swagger/index.html" -Color Blue
        Write-Host ""
